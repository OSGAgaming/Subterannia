   ...
   float x0, float x1, float x2)
        ...
        public static void DrawChain(Texture2D tex, int frameSize, int frameNum, Vector2 p1, Vector2 p2, float rotOffset = 0, float per = 1, Color color = default)
        {
            ...
            for (float i = 0; i < 1; i += (width / length) * per)
            {
                ...
                Main.spriteBatch.Draw(tex, lerp.ForDraw(), rect, color, rotation + rotOffset, rect.Size(), 1f, SpriteEffects.None, 0f);
            }
        }
        ...
        public static void DrawLine(Vector2 p1, Vector2 p2, Color tint = default, float lineWidth = 1f)
        {
            ...
            Main.spriteBatch.Draw(TextureAssets.MagicPixel.Value, p1, new Rectangle(0, 0, 1, 1), tint == default ? Color.White : tint, rotation, new Vector2(0f, 0.5f), new Vector2(length, lineWidth), SpriteEffects.None, 0f);
        }
